...
ao_freq_ratio = int(assim_freq/obs_freq_timestep)
obs_dens = 4
truthf = loadmat(truthfname)
ztruth = truthf['zens_times'][0:int(time_steps/obs_freq_timestep+1):ao_freq_ratio, :]
...
print('observations error std:', np.std(zt-zobs_total[0::ao_freq_ratio,:]))

# plt.plot(np.arange(0,20), zt[0:20,0])
# plt.plot(np.arange(0,20), zobs_total[0:20,0], '*')


def ensrf(ztruth, zics_total, zobs_total, Hk, obs_dens, obs_freq_timestep, assim_freq):
    # serial_update = 1
    ao_freq_ratio = int(assim_freq/obs_freq_timestep)

    # analysis period
    iobsbeg = 41
    iobsend = int(time_steps/assim_freq)+1
    nassim = int(time_steps/assim_freq)+1
    ...
    inflation_values = [1.05] #[1.01,1.03,1.05]
    ninf = len(inflation_values)
    localization_values = [240] # [120,240,480]
    ...
    prior_rmse = np.zeros((nassim,ninf,nloc))
    analy_rmse = np.zeros((nassim,ninf,nloc))
    prior_spread_rmse = np.zeros((nassim,ninf,nloc))
    analy_spread_rmse = np.zeros((nassim,ninf,nloc))    
    ...
    for iinf in range(ninf):
        ...
        for iloc in range(nloc):
            ...
            prior_spread = np.zeros((model_size, nassim))
            analy_spread = np.zeros((model_size, nassim))
            zeakf_prior = np.zeros((model_size, nassim))
            zeakf_analy = np.empty((model_size, nassim))
            ...
            step_beg = obsstep
            step_end = (iassim + 1) * assim_freq
            # start = time.time()
            ilag = 0
            zens_lag = np.zeros((ao_freq_ratio,ensemble_size,model_size))
            for istep in range(step_beg, step_end + 1):
                ...
                if istep % obs_freq_timestep == 0:
                    zens_lag[ilag,:,:] = zens
                    ilag += 1

            for iassim in range(1, nassim):
                print(iassim)

                # EnKF step
                obsstep = iassim * assim_freq + 1

                zeakf_prior[:, iassim] = np.mean(zens, axis=0)  # prior ensemble mean
                prior_spread[:, iassim] = np.std(zens, axis=0, ddof=1)

                zobs = zobs_total[((iassim-1)*ao_freq_ratio+1):iassim*ao_freq_ratio+1, :].flatten()
                ...
                # matrix update
                zens = EAKF_wzr(2, model_size, ensemble_size, ensemble_size,
                                nobsgrid, zens_lag, zens_lag, Hk, obs_error_var, localize, CMat, zobs)
...
err = ensrf(ztruth, zics_total, zobs_total, Hk, obs_dens, obs_freq_timestep, assim_freq)
