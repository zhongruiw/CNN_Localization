...
# get truth
time_steps = 200 * 20   # 360 days(1 day~ 200 time steps)
...
def ensrf(ztruth, zics_total, zobs_total):
    ...
    ensemble_size = 40
    group_size = 100
    ics = np.random.randint(0,3000,(ensemble_size,group_size))
    ...
    analy_rmses = np.array([])        
    prior_spread_rmses = np.array([])
    analy_spread_rmses = np.array([])
    prior_errs = np.array([])
    analy_errs = np.array([])

    for ig in range(group_size):

        zens = np.mat(zics_total[ics[:,ig], :])  # ensemble are drawn from ics set
        ...
        for iassim in range(0, nobstime):
            ...
            # serial EnSRF update
            # zens, zens_adm = EAKF_wzr(2, model_size, ensemble_size, adm_ensemble_size,
            #                 nobsgrid, zens, zens_adm, Hk, obs_error_var, localize, CMat, zobs)
        ...
        np.save('/scratch/lllei/F15/singobs/groups/kg_f_20d_g{:03d}.npy'.format(ig), kg_f)
        ...
        prior_rmses = np.append(prior_rmses,prior_rmse.squeeze())
        analy_rmses = np.append(analy_rmses,analy_rmse.squeeze())
        prior_spread_rmses = np.append(prior_spread_rmses,prior_spread_rmse.squeeze())
        analy_spread_rmses = np.append(analy_spread_rmses,analy_spread_rmse.squeeze())
        prior_errs = np.append(prior_errs, prior_err)
        analy_errs = np.append(analy_errs, analy_err)

    print('prior error = ')
    for ig in range(group_size):
        print('{0:.6f}'.format(prior_errs[ig]))
    # print('prior_2000 error = {0:.6f}'.format(prior_indeself_err))
    print('analysis error =')
    for ig in range(group_size):
        print('{0:.6f}'.format(analy_errs[ig]))
    # print('analy_2000 error = {0:.6f}'.format(analy_indeself_err))
    ...
    np.save('prior_rmse.npy',prior_rmse.reshape(group_size,nobstime))
    np.save('analy_rmse.npy',analy_rmse.reshape(group_size,nobstime))
    # np.save('prior_indeself_rmse.npy',prior_indeself_rmse)
    # np.save('analy_indeself_rmse.npy',analy_indeself_rmse)
    np.save('prior_spread_rmse.npy', prior_spread_rmse.reshape(group_size,nobstime))
    # np.save('prior_spread_inde_rmse.npy', prior_spread_inde_rmse)
    np.save('analy_spread_rmse.npy', analy_spread_rmse.reshape(group_size,nobstime))
    # np.save('analy_spread_inde_rmse.npy', analy_spread_inde_rmse)
    # np.save('/scratch/lllei/F16/singobs/2000/zens_times_prior.npy', zens_times)
    # np.save('/scratch/lllei/F16/singobs/2000/truth_times.npy', ztruth)
