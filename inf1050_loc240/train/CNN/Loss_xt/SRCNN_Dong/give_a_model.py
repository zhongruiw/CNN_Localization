...
def srcnn(**kwargs):
    ...
    class LocLoss(keras.losses.Loss):
        ...
        def call(self, kg_true, loc_f):
            ...
            # kg_pred = tf.math.multiply(self.kg_raw, tf.tile(loc_f, k))
            kg_pred = tf.math.multiply(self.kg_raw,
                                       tf.tile(tf.reshape(loc_f, [self.batch_size, self.model_size, 1, 1]), k))
    ...
    p1 = PeriodicPadding2D(padding=(22, 16))(inputs)
    x1 = layers.Conv2D(filters=128, kernel_size=(21, 9), strides=(1, 1), kernel_initializer='glorot_uniform',
                       activation='relu', padding='valid', use_bias=True)(p1)
    # p1 = layers.AveragePooling2D(pool_size=(2, 2), strides=None, padding='valid')(x1)
    x2 = layers.Conv2D(filters=64, kernel_size=(21, 21), strides=(1, 1), kernel_initializer='glorot_uniform',
                       activation='relu', padding='valid', use_bias=True)(x1)
    # p2 = layers.AveragePooling2D(pool_size=(2, 2), strides=None, padding='valid')(x2)
    outputs = layers.Conv2D(filters=1, kernel_size=(5, 5), strides=(1, 1), kernel_initializer='glorot_uniform',
                            activation='linear', padding='valid', use_bias=True)(x2)
    # x4 = layers.Flatten()(x3)
    # x5 = layers.Dense(240, name='Dense1')(x4)
    # outputs = layers.Dense(model_size, name="loc_f")(x5)
