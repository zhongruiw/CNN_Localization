...
obsfname = pjoin(data_dir, 'obs_ms3_240s_6h_5y_sptavd00625.mat')
...
def ensrf(ztruth, zics_total, zobs_total):
    ...
    for iassim in range(0, nobstime):
        ...
        # serial EnSRF update
        zens, zpmeans, phts, hphts = EAKF_wzr(182, model_size, ensemble_size, ensemble_size,
                                              nobsgrid, zens, zens, Hk, obs_error_var, localize, CMat, zobs)

        # calculate elf
        if iassim > ibegin:
            zt = np.array(ztruth[iassim, :])
            nume, deno, iinc_t, iinc_p = addelf(nume, deno, zt, zpmeans,
                                                model_size, nobsgrid, obs_density, obs_error_var,
                                                phts, hphts, np.array(Hk))
    ...
    # save
    np.save('elf.npy', elf)
    np.save('/scratch/lllei/spatially_averaged/elf/avrgd_00625/based_on_gc240/debug_481/seri_up/inc_t.npy', inc_t)
    np.save('/scratch/lllei/spatially_averaged/elf/avrgd_00625/based_on_gc240/debug_481/seri_up/inc_p.npy', inc_p)
...
@jit(nopython=True)
def addelf(nume, deno, zt, zpmeans, model_size, nobsgrid, obs_density, obs_error_var, phts, hphts, Hk):
