...
# version: 2.8
# update: add gamma obs; add loc_dep selection; multiple inf,git repo; allow deflation; fix bug, distance 481,not 480
...
# make forward operator H, options:'single','average','gamma_weighted'
theta = 80
gammafname = pjoin(data_dir, 'gamma_k2_theta{:d}.npy'.format(theta))
gamma = np.load(gammafname)
Hk = H('gamma_weighted', model_size, obs_density, gamma=gamma, theta=theta)

# get observation
obsfname = pjoin(data_dir, 'obs_ms3_err1_240s_6h_25y_gamma{:d}.mat'.format(theta))
obsf = loadmat(obsfname)
zobs_total = obsf['zobs_total'][eva_bg:eva_ed, :]  # 360 days
...
def ensrf(ztruth, zics_total, zobs_total, Hk):
    serial_update = 1
    ...
    elif elf_mode == 'loc_dep':
        nume = np.zeros((model_size))
        deno = np.zeros((model_size))
        elf = np.zeros((model_size))
    else:
    ...
    for iassim in range(0, nobstime):
        ...
        if iassim >= ibegin:
            ...
            if elf_mode == 'dis_dep':
                ...
                nume, deno  = addelf_loc(nume, deno, zt, zp,
                                        model_size, nobsgrid, obs_density, obs_error_var, Pb, Hk)
                # inc_t = np.append(inc_t, iinc_t, axis=0)
                # inc_p = np.append(inc_p, iinc_p, axis=0)
            else:
    ...
    # save
    np.save('elf.npy', elf)
    # np.save('/scratch/lllei/inf1050_loc240/train/elf/successive/inf1000/inc_t.npy', inc_t)
    # np.save('/scratch/lllei/inf1050_loc240/train/elf/successive/inf1000/inc_p.npy', inc_p)
...
@jit(nopython=True)
def addelf_loc(nume, deno, zt, zp, model_size, nobsgrid, obs_density, obs_error_var, Pb, Hk):
    # iinc_t = np.zeros((int(nobsgrid * 2), int(model_size / 2)+1))
    # iinc_p = np.zeros((int(nobsgrid * 2), int(model_size / 2)+1))
    for iob in range(0, nobsgrid):
        hk = Hk[iob, :]
...
err = ensrf(ztruth, zics_total, zobs_total, Hk)
